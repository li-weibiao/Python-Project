...
# 注意：在使用第三方模块时，首先需要使用
# pip install命令安装该模块，例如，安装
# qrcode模块，可以在Python命令窗口中执行
# 以下命令：
# pip install qrcode
# 在本项目中有两个第三方模块，为qrcode和pystrich
...
root = tkinter.Tk()  # tkinter模块为python的标准图形界面接口。本代码的目的是建立根窗口,Tk一个大写，一个小写。
# 初始化数据
number = "1234567890"
	...
	if len(instr) != 0:  # 输入数据的长度不为零
		...
		if showorder == 1:  # 验证方式1，数字格式，不限位数，大于零的整数
			if str.isdigit(instr):  # 验证是否为数字
				...
				print("\033[1;31;40m 输入非法，请重新输入！！\033[0m")  # 要求重新输入
				return "0"  # 函数返回值为“0”
		...
		if showorder == 3: # 验证方式3，要求数字格式且输入数字位数有要求
			if str.isdigit(instr):  # 验证是否为数字
				if len(instr) != length: # 验证输入数字是不是指定位数
		...
		print("\033[1;31;40m 输入为空，请重新输入！！\033[0m")  # 输入为空，要求重新输入
		return "0"  # 函数返回值为“0”
...
def wfile(sstr, sfile, typeis, smsg, datapath):
	...
	for i in range(len(wrlist)):  # 按条循环读取防伪码数据
		...
		ptada = pdata + wdata  # 将单条防伪码存储到pdata变量
	file.close()  # 关闭文件
...
def scode1(schoice):
	...
	incount = inputbox("\033[1;32m     请输入您要生成验证码的数量:\33[0m", 1, 0)  # 其中，1，0为inputbox(盒子)位置参数“showorder”和“length”，不可省略
...
def scode4(schoice):
	...
	while not str.isalpha(intype) or len(intype) != 3:  # isalpha()函数检测字符串中是否只由“字母”组成
		intype = inputbox("\033[1;32m     请输入数据分析编号（3位字母）:\33[0m", 2, 3)
...
def scode6(schoice):
	...
	remove_digits = strset.maketrans("", "", digits)  # 用maketrans方法创建删除数字的字符映射转换表, digits是可选参数，表示要删除的字符组成的字符串
...
def scode7(schoice):
	...
	for j in range(int(incount)):   # 批量生成条形码
		...
		oddsum = int( barcode[0]) + int(barcode[2]) + int(barcode[4]) + int(barcode[6]) + int(barcode[8]) + int(barcode[10])  # 奇数位
...
def input_validation(insel):
	if str.isdigit(insel):  # Python isdigit() 方法检测字符串是否只由数字组成。如果字符串只包含数字则返回 True 否则返回 False。
		insel = int(insel)
		# if insel == 0:
...
def codeprint(cstr, cint):
	...
	for i in range(int(cint)):
		strbook = str1 + str2 + random.choice(letter) + random.choice(letter) + str2 + str3
		print(strbook)
