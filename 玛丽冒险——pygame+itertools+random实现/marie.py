...
class MyMap():
	...
	def __init__(self, x, y):
		# 加载背景图片
		self.bg = pygame.image.load("image/bg.png").convert_alpha()  # python 给图像添加透明度(alpha通道)，Convert()所指的“格式”并非指文件格式工（如 png，jpeg，gif），它是所谓的“像素格式”。
		self.x = x
		self.y = y
	...
	# 更新地图
	def map_update(self):
		SCREEN.blit(self.bg, (self.x, self.y))  # pygame用blit()实现动画效果
...
class Marie():
	def __init__(self):
		# 初始化小玛丽矩形
		self.rect = pygame.Rect(0, 0, 0, 0)  # Python中的rect，他其实在Python中就是像处理矩形（rect对象）一样处理游戏元素
		...
		# 小玛丽动图索引
		self.marieIndex = 0
		self.marieIndexGen = cycle([0, 1, 2])  # Python 用cycle()循环迭代，cycle()函数重复循环一组值，可用它循环数据集标识符对数据集进行分组。
		# 加载小玛丽图片
		self.adventure_img = (
