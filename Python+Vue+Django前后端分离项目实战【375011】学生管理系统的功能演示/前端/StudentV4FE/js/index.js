    ...
    methods: {
        ...
        getStudents: function () {
            ...
            axios
                ...
                .then(function (res) {      //此处是与后端django相关的函数代码
                    //请求成功后执行的函数
                    if (res.data.code === 1) {
                        //把数据给students
                        ...
                        //失败的提示！
                        that.$message.error(res.data.msg);  //django里的views.py里的变量
        ...
        queryStudents() {
            ...
            axios
                ...
                .then(function (res) {  //这个.then函数都是与后端django有关的代码
                    if (res.data.code === 1) {
                        //把数据给students
                        that.students = res.data.data;
                        //获取返回记录的总行数
        ...
        deleteStudent(row) {
            ...
            this.$confirm('是否确认删除学生信息【学号：' + row.sno + '\t姓名：' + row.name + '】信息？',
                ...
                let that = this
                //调用后端接口
                axios.post(that.baseURL + 'student/delete/', { sno: row.sno })  //sno: row.sno是后面传递的数据
                    .then(res => {
                        if (res.data.code === 1) {
                            //获取所有学生信息
                            that.students = res.data.data;
