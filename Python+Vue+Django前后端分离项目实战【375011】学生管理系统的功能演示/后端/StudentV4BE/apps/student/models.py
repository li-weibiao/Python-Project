...
class Student(models.Model):
    ...
    sno = models.IntegerField(db_column="SNo", primary_key=True, null=False) # IntegerField整型，db_column作为数据表内的列名，学号，不允许为空，主键
    name = models.CharField(db_column="SName", max_length=100, null=False)  # CharField字符串字段，姓名，最长100个字符，不允许为空
    gender = models.CharField(db_column="Gender",max_length=100,choices=gender_choices) # 性别，选项选择
    birthday = models.DateField(db_column="Birthday", null=False) # DateField日期类型，出生日期，不允许为空
    ...
    # __str__方法
    def __str__(self):  # __str__方法来展示直观的信息
